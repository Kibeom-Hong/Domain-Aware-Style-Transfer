...
from apis.utils.style_decorator import StyleDecorator
...
def gaussian_blur(x, alpha):
    ...
    for batch in range(B):
        ...
        gaussian_kernel = (1./(2.*math.pi*variance)) * \
            torch.exp(diff / (2*variance))
        gaussian_kernel = gaussian_kernel / torch.sum(gaussian_kernel)
        gaussian_kernel = gaussian_kernel.view(1, 1, kernel_size, kernel_size)
        gaussian_kernel = gaussian_kernel.repeat(channels, 1, 1, 1)
        gaussian_kernel = gaussian_kernel.cuda()
        gaussian_filter.weight.data = gaussian_kernel
        output = gaussian_filter(torch.nn.functional.pad(
            x[batch].unsqueeze(0), (mean, mean, mean, mean), "replicate"))
...
class AdaIN(nn.Module):
    ...
    def forward(self, content, style, style_strength=1.0, eps=1e-5):
        ...
        normalized_content = (content.view(b, c, -1) -
                              content_mean)/(content_std+eps)

        stylized_content = (normalized_content * style_std) + style_mean

        output = (1-style_strength)*content + style_strength * \
            stylized_content.view(b, c, h, w)
        return output
...
class VGGEncoder(nn.Module):
    def __init__(self, vgg):
        ...
        self.conv0.weight = nn.Parameter(
            torch.FloatTensor(vgg.modules[0].weight))
        self.conv0.bias = nn.Parameter(torch.FloatTensor(vgg.modules[0].bias))

        ###Level1###
        self.conv1_1 = nn.Conv2d(3, 64, 3, 1, 0)
        self.conv1_1.weight = nn.Parameter(
            torch.FloatTensor(vgg.modules[2].weight))
        self.conv1_1.bias = nn.Parameter(
            torch.FloatTensor(vgg.modules[2].bias))

        self.conv1_2 = nn.Conv2d(64, 64, 3, 1, 0)
        self.conv1_2.weight = nn.Parameter(
            torch.FloatTensor(vgg.modules[5].weight))
        self.conv1_2.bias = nn.Parameter(
            torch.FloatTensor(vgg.modules[5].bias))

        ###Level2###
        self.conv2_1 = nn.Conv2d(64, 128, 3, 1, 0)
        self.conv2_1.weight = nn.Parameter(
            torch.FloatTensor(vgg.modules[9].weight))
        self.conv2_1.bias = nn.Parameter(
            torch.FloatTensor(vgg.modules[9].bias))

        self.conv2_2 = nn.Conv2d(128, 128, 3, 1, 0)
        self.conv2_2.weight = nn.Parameter(
            torch.FloatTensor(vgg.modules[12].weight))
        self.conv2_2.bias = nn.Parameter(
            torch.FloatTensor(vgg.modules[12].bias))

        ###Level3###
        self.conv3_1 = nn.Conv2d(128, 256, 3, 1, 0)
        self.conv3_1.weight = nn.Parameter(
            torch.FloatTensor(vgg.modules[16].weight))
        self.conv3_1.bias = nn.Parameter(
            torch.FloatTensor(vgg.modules[16].bias))

        self.conv3_2 = nn.Conv2d(256, 256, 3, 1, 0)
        self.conv3_2.weight = nn.Parameter(
            torch.FloatTensor(vgg.modules[19].weight))
        self.conv3_2.bias = nn.Parameter(
            torch.FloatTensor(vgg.modules[19].bias))

        self.conv3_3 = nn.Conv2d(256, 256, 3, 1, 0)
        self.conv3_3.weight = nn.Parameter(
            torch.FloatTensor(vgg.modules[22].weight))
        self.conv3_3.bias = nn.Parameter(
            torch.FloatTensor(vgg.modules[22].bias))

        self.conv3_4 = nn.Conv2d(256, 256, 3, 1, 0)
        self.conv3_4.weight = nn.Parameter(
            torch.FloatTensor(vgg.modules[25].weight))
        self.conv3_4.bias = nn.Parameter(
            torch.FloatTensor(vgg.modules[25].bias))

        ###Level4###
        self.conv4_1 = nn.Conv2d(256, 512, 3, 1, 0)
        self.conv4_1.weight = nn.Parameter(
            torch.FloatTensor(vgg.modules[29].weight))
        self.conv4_1.bias = nn.Parameter(
            torch.FloatTensor(vgg.modules[29].bias))
    ...
    def encode(self, x, skips):
        ...
        HH = out - F.interpolate(pooled_feature,
                                 size=[resize_w, resize_h], mode='nearest')
        skips['pool1'] = HH
        ##################################
        if is_maxpool:
...
class VGGDecoder(nn.Module):
    ...
    def decode(self, x, content_skips, style_skips, level, alphas=[], is_recon=True):
        ...
        if len(alphas) > 0:
            a1, a2, a3 = alphas[:, 0], alphas[:,
                                              1], alphas[:, 2]  # a1는 128~ a3은512
        ...
        resize_w, resize_h = content_skips['conv3_4'].size(
            2), content_skips['conv3_4'].size(3)
        unpooled_feat = F.interpolate(
            out, size=[resize_w, resize_h], mode='nearest')

        HH = content_skips['pool3']

        intermediate_stylized = feature_wct_simple(
            content_skips['conv3_4'], style_skips['conv3_4'])
        ...
        resize_w, resize_h = content_skips['conv2_2'].size(
            2), content_skips['conv2_2'].size(3)
        ...
        intermediate_stylized = feature_wct_simple(
            content_skips['conv2_2'], style_skips['conv2_2'])
        stylized_HH, _ = get_LL_HH(intermediate_stylized)
        unpooled_HH, _ = get_LL_HH(unpooled_feat)
        ...
        resize_w, resize_h = content_skips['conv1_2'].size(
            2), content_skips['conv1_2'].size(3)
        out = self.relu(self.conv2_1(self.pad(out)))
        unpooled_feat = F.interpolate(
            out, size=[resize_w, resize_h], mode='nearest')

        intermediate_stylized = feature_wct_simple(
            content_skips['conv1_2'], style_skips['conv1_2'])
    ...
    def reconstruct(self, x):
        ...
        out = self.relu(self.conv4_1(self.pad(x)))
        out = F.interpolate(
            out, size=[out.size(2), out.size(3)], mode='nearest')
        out = self.relu(self.conv3_4(self.pad(out)))
        out = self.relu(self.conv3_3(self.pad(out)))
        out = self.relu(self.conv3_2(self.pad(out)))
        out = self.relu(self.conv3_1(self.pad(out)))
...
class Baseline_net(nn.Module):
    ...
    def forward(self, content, style, content_segment, style_segment, is_recon=True, alpha=1, alphas=[], type='photo'):
        ...
        ####Style transfer####
        transformed_feature = alphas.mean() * self.decorator(content_feat, style_feat) + \
            (1-alphas.mean())*feature_wct_simple(content_feat, style_feat)

        ####Decode####
        stylized_image = self.decode(
            transformed_feature, content_skips, style_skips, 1, alphas, is_recon=is_recon)
...
class MultiScaleImageDiscriminator(nn.Module):
    def __init__(self, nc=3, ndf=64):
        ...
        self.conv5 = nn.Sequential(
            # 16->8
            spectral_norm(
                nn.Conv2d(ndf*8, self.output_dim, 4, 2, 1, bias=False)),
        )
