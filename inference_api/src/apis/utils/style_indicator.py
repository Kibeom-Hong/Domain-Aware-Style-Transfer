...
class DA_Net_v3(nn.Module):
    ...
    def forward(self, inputs, level=1):
        if level == 1:
            structure_feat = self.conv1(
                get_LL_HH(inputs)[0]).view(inputs.size(0), -1)
...
class New_DA_Net_v1(nn.Module):
    ...
    def forward(self, inputs, level=1):
        if level == 1:
            texture_feat = self.linear1(gram_matrix(inputs).view(
                inputs.size(0), -1)).unsqueeze(2).unsqueeze(3).repeat(1, 1, 64, 64)
            structure_feat = self.conv1(inputs)
            concat_feat = torch.cat([structure_feat, texture_feat], dim=1)
            x = self.shared_conv(concat_feat).squeeze()
            return x
        elif level == 2:
            texture_feat = self.linear2(gram_matrix(inputs).view(
                inputs.size(0), -1)).unsqueeze(2).unsqueeze(3).repeat(1, 1, 64, 64)
            ...
            texture_feat = self.linear3(gram_matrix(inputs).view(
                inputs.size(0), -1)).unsqueeze(2).unsqueeze(3).repeat(1, 1, 64, 64)
...
class New_DA_Net_v2(nn.Module):
    ...
    def forward(self, inputs, level=1):
        ...
        texture_feat_1 = self.linear1(gram_matrix(in1).view(
            in1.size(0), -1)).unsqueeze(2).unsqueeze(3).repeat(1, 1, 64, 64)
        structure_feat_1 = self.conv1(in1)
        concat_feat_1 = torch.cat([structure_feat_1, texture_feat_1], dim=1)

        texture_feat_2 = self.linear2(gram_matrix(in2).view(
            in2.size(0), -1)).unsqueeze(2).unsqueeze(3).repeat(1, 1, 64, 64)
        structure_feat_2 = self.conv2(in2)
        concat_feat_2 = torch.cat([structure_feat_2, texture_feat_2], dim=1)

        texture_feat_3 = self.linear3(gram_matrix(in3).view(
            in3.size(0), -1)).unsqueeze(2).unsqueeze(3).repeat(1, 1, 64, 64)
        structure_feat_3 = self.conv3(in3)
        concat_feat_3 = torch.cat([structure_feat_3, texture_feat_3], dim=1)

        total_feat = torch.cat(
            [concat_feat_1, concat_feat_2, concat_feat_3], dim=1)
...
class New_DA_Net_v3(nn.Module):
    ...
    def forward(self, inputs, level=1):
        ...
        if level == 1:
            texture_feat = self.linear1(gram_matrix(inputs).view(
                inputs.size(0), -1)).unsqueeze(2).unsqueeze(3).repeat(1, 1, w, h)
