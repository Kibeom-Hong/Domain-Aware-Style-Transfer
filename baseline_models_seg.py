...
def gaussian_blur(x, alpha):
		...
		for batch in range(B):
			#kernel_size = int(int(int(alpha[batch].item()*16)/2)*2)+1
			...
			#kernel_size = int(int(int(alpha[batch].item()*4)/2)*2)+1
...
class VGGDecoder(nn.Module):
	def __init__(self):
		...
		#self.transformation3 = Transformation_v6(256)
		#self.transformation2 = Transformation_v6(128)
		#self.transformation1 = Transformation_v6(64)
	...
	def decode(self, x, content_skips, style_skips, level, content_segment, style_segment, label_set, label_indicator, alphas=[]):
		...
		#unpooled_feat = F.interpolate(out, scale_factor=2, mode='nearest')		
		...
		#intermediate_stylized = feature_wct(content_skips['conv3_4'], style_skips['conv3_4'], content_segment, style_segment, label_set, label_indicator)
		...
		#out = unpooled_feat + (gaussian_blur(intermediate_stylized, (a3)) )
		#out = unpooled_feat + (gaussian_blur(origin_HH, (a3)) )
		#out = unpooled_feat + stylized_HH*(1-a3)
		#out = unpooled_feat
		#out = unpooled_feat + (a3 * (gaussian_blur(stylized_HH, (a3))) + (1-a3)*(gaussian_blur(intermediate_stylized, (a3)) ))
		...
		#out = self.adain(out, style_skips['conv3_1']) ##AdaIN##
		out = feature_wct_simple(out, style_skips['conv3_1'])
		#out = feature_wct(out, style_skips['conv3_1'], content_segment, style_segment, label_set, label_indicator)
		...
		#intermediate_stylized = feature_wct(content_skips['conv2_2'], style_skips['conv2_2'], content_segment, style_segment, label_set, label_indicator)
		...
		#out = unpooled_feat + (gaussian_blur(intermediate_stylized, (a2)) )
		#out = unpooled_feat + (gaussian_blur(origin_HH, (a2)) )
		#out = unpooled_feat + stylized_HH*(1-a2)
		#out = unpooled_feat
		#out = unpooled_feat + (a2 * (gaussian_blur(stylized_HH, (a2)) ) + (1-a2)*(gaussian_blur(intermediate_stylized, (a2)) ))
		
		out = self.relu(self.conv2_2(self.pad(out)))
		
		#out = self.adain(out, style_skips['conv2_1']) ##AdaIN##
		out = feature_wct_simple(out, style_skips['conv2_1'])
		#out = feature_wct(out, style_skips['conv2_1'], content_segment, style_segment, label_set, label_indicator)
		...
		#intermediate_stylized = self.adain(content_skips['conv1_2'], style_skips['conv1_2'])
		#intermediate_stylized = feature_wct(content_skips['conv1_2'], style_skips['conv1_2'], content_segment, style_segment, label_set, label_indicator)
		intermediate_stylized = feature_wct_simple(content_skips['conv1_2'], style_skips['conv1_2'])
		
		HH = content_skips['pool1']
		#out = unpooled_feat + HH + unpooled_HH
		...
		#out = unpooled_feat + (gaussian_blur(intermediate_stylized, (a1)) )
		#out = unpooled_feat + (gaussian_blur(origin_HH, (a1)) )
		#out = unpooled_feat + stylized_HH*(1-a1)
		#out = unpooled_feat
		#out = unpooled_feat + (a1 * (gaussian_blur(stylized_HH, (a1)) ) + (1-a1)*(gaussian_blur(intermediate_stylized, (a1)) ))
		
		out = self.relu(self.conv1_2(self.pad(out)))
		
		#out = self.adain(out, style_skips['conv1_1']) ##AdaIN## -> WCT로 변경해야함
		#out = feature_wct(out, style_skips['conv1_1'], content_segment, style_segment, label_set, label_indicator)
...
class Baseline_net(nn.Module):
	def __init__(self, pretrained_vgg=None):#, device='cuda:0'):
		super(Baseline_net, self).__init__()

		#self.device = torch.device(device)
		
		self.encoder = VGGEncoder(pretrained_vgg).cuda()#.to(self.device)
		self.decoder = VGGDecoder().cuda()#.to(self.device)
		self.decorator = StyleDecorator().cuda()#.to(self.device)

	def encode(self, x, skips):
		return self.encoder.encode(x, skips)

	def decode(self, x, content_skips, style_skips, level, content_segment, style_segment, label_set, label_indicator, alphas):
		return self.decoder.decode(x, content_skips, style_skips, level, content_segment, style_segment, label_set, label_indicator, alphas)
	...
	def forward(self, content, style, content_segment, style_segment, is_recon=True, alpha=1, alphas=[], type='photo'):
		...
		####Style transfer####
		#transformed_feature = self.decorator(content_feat, style_feat)
		#transformed_feature = feature_wct_simple(content_feat, style_feat)
		transformed_feature = alphas.mean() * self.decorator(content_feat, style_feat) + (1-alphas.mean())*feature_wct(content_feat, style_feat, content_segment, style_segment, label_set, label_indicator, device='cuda:0')

		####Decode####
		stylized_image = self.decode(transformed_feature, content_skips, style_skips, 1, content_segment, style_segment, label_set, label_indicator, alphas)
...
class ImageDiscriminator(nn.Module):
	def __init__(self, nc=3, ndf=64, use_noise=False, noise_sigma=None):
